...
const setupCommands = require('./bot/command'); // 👈 naya import
...
// ---------------- COMMANDS REGISTER ----------------
setupCommands(bot); // 👈 pura /start,/help,/more,resume yahan se load ho jayega
  ...
  async () => {
    try {
      ...
      for (const user of users) {
        ...
        const newJobs = jobs.filter((j) => !seen.includes(j.url));
        if (!newJobs.length) continue;

        let msg = '🔥 New Jobs Update\n\n';
